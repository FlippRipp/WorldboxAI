...
- "The roads between cities are stalked by shadow-beasts that hunt at dusk and dawn"
- "No merchant will trade with strangers without a blood-sealed contract"
- "People in this world don't trust each other — betrayal and backstabbing are the norm"
- "Every settlement must pay the harvest tithe to the forest spirits, or the land turns barren"
...
class WorldRulesStep(Step):
    id = "world_rules"
    label = "World Rules"
    description = "Define the genre, tone, magic level, tech era, and practical world constraints that shape daily life."
    after = None
    guidance = _GUIDANCE
    schema = {
        "genre": {"type": "string", "label": "Genre"},
        "tone": {"type": "string", "label": "Tone"},
        "magic_level": {"type": "select", "label": "Magic Level", "options": ["none", "rare", "common", "ubiquitous"]},
