"""Procedural overworld generation: ``WorldMapGenerator`` plus the
``generate_map`` / ``generate_multilayer_map`` entry points the ``world_map``
generator spec (``registry.py``) and ``generation/maps.py`` wrap. The shared
map types live in ``wbworldgen.mapmodel``; this module owns only the
placement/edge/region/road algorithms."""
...
class WorldMapGenerator:
    ...
    def _generate_with_terrain(
        ...
        if deferred_anchored:
            placed_by_name = {join_key(n.name): n for n in nodes if n.name}
            ...
            for rname, loc in deferred_anchored:
                anchor = placed_by_name.get(join_key(loc.get("part_of", "")))
                if anchor is not None and loc.get("relation") == "inside":
                    contained = anchor.contained_locations
                    if join_key(loc.get("name", "")) not in {
                            join_key(c.get("name", "")) for c in contained}:
                ...
                placed_by_name[join_key(node.name)] = node
