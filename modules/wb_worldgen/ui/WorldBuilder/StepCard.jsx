...
export default function StepCard({ step, state, onApprove, onReroll, onAddNote, onRerollItem, loading, onEnrich, onEnrichCommit, worldId, worldState }) {
  ...
  const handleRerollItem = useCallback(async (fieldKey, index, opts = {}) => {
    if (!onRerollItem) return;
    const { subfield, note } = opts;
    const items = editedData?.[fieldKey] || [];
    const key = subfield ? `${fieldKey}:${index}:${subfield}` : `${fieldKey}:${index}`;
    setRerollingKey(key);
    try {
      const newItem = await onRerollItem(step.id, fieldKey, index, items, { subfield, note });
      if (newItem !== null && newItem !== undefined) {
        setEditing(true);
        setEditedData((prev) => {
          const next = [...(prev?.[fieldKey] || [])];
          if (subfield) {
            next[index] = { ...(next[index] || {}), [subfield]: newItem };
          } else {
            next[index] = newItem;
          }
