...
/** Small spinner / reroll button shared by list-item reroll controls. */
function RerollButton({ active, onClick, disabled, title }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      title={title}
      className="text-purple-400 hover:text-purple-300 disabled:opacity-40 px-1.5"
    >
      {active ? (
        <span className="inline-block w-3.5 h-3.5 border-2 border-purple-400/30 border-t-purple-400 rounded-full animate-spin align-middle" />
      ) : (
        '🔄'
      )}
    </button>
...
export function StepField({ fieldKey, fieldSchema, value, onChange, disabled, onRerollItem, rerollingKey }) {
  ...
  // Ephemeral per-entry guidance notes for structured-list reroll, keyed by index.
      ...
      <div className="space-y-3">
        {items.map((item, i) => (
          <div key={i} className="p-3 bg-gray-800/50 border border-gray-700 rounded space-y-2">
            {canReroll && (
              <div className="flex items-center gap-2 pb-2 mb-1 border-b border-gray-700/60">
                <button
                  onClick={() => onRerollItem(fieldKey, i, { note: itemNotes[i] })}
                  disabled={anyRerolling}
                  title={`Re-roll this entire ${entryLabel} with AI`}
                  className="flex items-center gap-1.5 text-xs px-2 py-1 bg-purple-900/40 hover:bg-purple-800/50 border border-purple-700/40 rounded text-purple-200 disabled:opacity-40"
                >
                  {rerollingKey === `${fieldKey}:${i}` ? (
                    <span className="inline-block w-3 h-3 border-2 border-purple-300/30 border-t-purple-300 rounded-full animate-spin" />
                  ) : (
                    '🔄'
                  )}
                  Re-roll {entryLabel}
                ...
                <input
                  ...
                  placeholder="note to steer this re-roll..."
                  disabled={anyRerolling}
                  className="flex-1 bg-gray-900 border border-gray-700 rounded px-2 py-1 text-xs text-gray-300 focus:border-purple-500 focus:outline-none"
                />
              </div>
            )}
                ...
                return (
                  <div key={subKey}>
                    <div className="flex items-center gap-1">
                      ...
                      {canReroll && (
                        <RerollButton
                          active={rerollingKey === `${fieldKey}:${i}:${subKey}`}
                          onClick={() => onRerollItem(fieldKey, i, { subfield: subKey, note: itemNotes[i] })}
                          disabled={anyRerolling}
                          title={`Re-roll just ${typeof sub === 'object' ? sub.label : subKey}`}
                        />
                      )}
                    </div>
